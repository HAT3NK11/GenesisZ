...
namespace GenesisZ.Items.Bosses
{
    ...
    public class TMMCBoss : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("Marketron");
        ...
        public override void AI()
        {
            ...
            {
                frame = 0;
                MoveTowards(npc, target, (float)(distance > 300 ? 16f : 10f), 30f);
                npc.netUpdate = true;
            }
            ...
            if ((double)npc.ai[0] % (Main.expertMode ? 100 : 150) == 0 && !stunned && !fastSpeed)
            {
                ...
                if (attackTimer <= 2)
                {
                    ...
                    {
                        Projectile.NewProjectile(shootPos.X + (float)(-100 * player.direction) + (float)Main.rand.Next(-80, 41), shootPos.Y - (float)Main.rand.Next(-60, 40), shootVel.X, shootVel.Y, mod.ProjectileType("TMMCBossProjectile"), npc.damage / 3, 5f);
                    }
                }
                else
                {
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            if (choice == 0)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("BauxiteOre"),30);
                            }
            else if (choice == 1)
            {
                Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("CryoliteOre"),30);
            }
        }
    }
}
